...
class windowclass():
	def __init__(self, master):
		self.master = master
		Label(self.master,text="RACE TURTLE GAME",width="300",height="2",font=("calibri",13)).pack()
		Label(self.master,text="").pack()
		Label(self.master,text="Enter you name: ").pack()
		self.entryname=Entry(master)
		self.entryname.pack()
		Label(self.master,text="").pack()
		Button(master,text="Press",height=1,width=10,command=self.command1).pack()
...
class nhaptien:
	def __init__(self,master):
		...
		Label(master,text="Choose charactor (1-4): ").grid(row=2)
		...
		Button(master, text='Enter Money', command=self.show).grid(row=4, column=1, sticky=W, pady=4)
		Button(master, text='Play', command=self.command3_1).grid(row=4, column=2, sticky=W, pady=4)
		Button(master,text='shop',command=self.command3_2).grid(row=4,column=3,sticky=W,pady=4)
	def show(self):
		global nhaptiencuoc
		global nhaptiennap
		global chooserua
		nhaptiennap=int(self.e1.get())
		nhaptiencuoc=int(self.e2.get())
		chooserua=int(self.e3.get())
		if nhaptiennap<nhaptiencuoc:
...
def playgame1():
    ...
    if chondang!=0:
        ...
        if chondang==3:shape="moto.png";shape1="moto1.png"
        win.addshape(shape)
        win.addshape(shape1)
  ...
  #Rua Vang----------------------------------------     
   ...
   #Rua Den-------------------------------------------   
    ...
    if ketqua==chosenone:
        ...
        score=nhaptiennap-nhaptiencuoc
    game = Tk()
    def close(): 
         exit()
    #Result
    ...
    text = screen.create_text(275,60,toadoxext =playername, fill="blue", font=("Arial", 15))
    #diem
    ...
    #button exit
    btn_exit = Button(master=game, text='Back', font=("Arial", 14, "bold"),bg="black", fg="green", width=10, height=1, command=returnplay)
    btn_exit.pack()
