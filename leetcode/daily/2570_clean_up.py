def mergeArrays(nums1, nums2):
    res = []    
    n1, n2 = 0, 0
    while n1 < len(nums1) or n2 < len(nums2):
        id1 = nums1[n1][0] if n1 < len(nums1) else float('inf')
        id2 = nums2[n2][0] if n2 < len(nums2) else float('inf')
        
        if id1 == id2:
            res.append([id1, nums1[n1][1] + nums2[n2][1]])
            n1 += 1
            n2 += 1
        elif id1 < id2:
            res.append([id1, nums1[n1][1]])
            n1 += 1
        else:
            res.append([id2, nums2[n2][1]])
            n2 += 1
    return res

print(mergeArrays([[148,597],[165,623],[306,359],[349,566],[403,646],[420,381],[566,543],[730,209],[757,875],[788,208],[932,695]], [[74,669],[87,399],[89,165],[99,749],[122,401],[138,16],[144,714],[148,206],[177,948],[211,653],[285,775],[309,289],[349,396],[386,831],[403,318],[405,119],[420,153],[468,433],[504,101],[566,128],[603,688],[618,628],[622,586],[641,46],[653,922],[672,772],[691,823],[693,900],[756,878],[757,952],[770,795],[806,118],[813,88],[919,501],[935,253],[982,385]]))
print(mergeArrays([[1,2],[2,3],[4,5]], [[1,4],[3,2],[4,1]]))
# just test edit file
